...
class ImportAjaxHandler implements Hookable {
	...
	public function hooks() {
		add_action( 'wp_ajax_' . self::AJAX_ACTION, [ $this, 'handle_ajax_action_import' ] );
	}
	...
	public function handle_ajax_action_import() {
		...
		wp_send_json_error( [ 'message' => __( 'Missing or invalid "stage" parameter!', 'flexible-shipping-import-export' ) ] );
	}
	...
	private function parse( $zone_id ) {
		...
		$data         = $this->remove_bom( $this->get_post_value_as_string( 'data' ) );
	...
	private function import( $zone_id ) {
		$shipping_methods_to_import = json_decode( $this->get_post_value_as_string( 'data' ), true );
	...
	private function prepare_conditions_fields( array $rules, ImportProcessor $conditions_preparing ) {
		foreach ( $rules as $rule_id => $rule ) {
			...
			foreach ( $conditions as $condition_id => $condition ) {
				$conditions[ $condition_id ] = $conditions_preparing->process_condition( $condition, [] ); // TODO: mappings.
			}
	...
	private function disable_shipping_method( WC_Shipping_Method $shipping_method, WC_Shipping_Zone $zone ) {
		...
		if ( $wpdb->update( "{$wpdb->prefix}woocommerce_shipping_zone_methods", [ 'is_enabled' => 0 ], [ 'instance_id' => $shipping_method->instance_id ] ) ) {
			...
			foreach ( $zone_shipping_methods as $shipping_method ) {
				if ( $shipping_method instanceof ShippingMethodSingle ) {
					$shipping_methods[] = [
						...
						'rules_count'    => count( $shipping_method->get_instance_option( CommonMethodSettings::METHOD_RULES, [] ) ),
						...
						// Translators: method title, new line, rules count.
							_n(
								...
								count( $shipping_method->get_instance_option( CommonMethodSettings::METHOD_RULES, [] ) ),
