...
class CSVDataParser implements DataParser {
	...
	private function normalize_shipping_methods_data( array $shipping_methods_data ) {
		foreach ( array_values( $shipping_methods_data ) as $key => $shipping_method_data ) {
			$shipping_method_data->update_setting( ShippingMethodData::METHOD_ID, (int) $key + 1 );
			$method_rules = $shipping_method_data->get_method_rules();
	...
	private function process_method_settings( array $single_row, ShippingMethodData $shipping_method_data ) {
		foreach ( $this->method_settings->get_settings_fields( [], true ) as $field_name => $field_settings ) {
			if ( RulesSettingsField::FIELD_TYPE === $field_settings['type'] ) {
				continue;
			}
			$csv_row_field_name = $this->get_label_shipping_method_field( $field_name );
			$value = isset( $single_row[ $csv_row_field_name ] ) ? trim( $single_row[ $csv_row_field_name ] ) : '';
			if ( '' !== $value ) {
				$shipping_method_data->update_setting( $field_name, $this->decode_json_if_needed( $value ) );
			}
		}
	 ...
	 * @return string|array
	 */
	private function decode_json_if_needed( $value ) {
	...
	private function process_rule_costs( array $rule, array $single_row ) {
		return $this->process_fields( $rule, $this->rule_costs_fields, [ $this, 'get_label_rule_costs_field' ], $single_row );
	}
	...
	private function process_rule_additional_costs( array $rule, array $single_row ) {
		...
		if ( $additional_costs_number && count( $this->rule_additional_costs_fields ) ) {
			...
			$additional_costs[ $additional_costs_number ] = $this->process_fields( [], $this->rule_additional_costs_fields, [ $this, 'get_label_additional_cost_field' ], $single_row );
		}
		...
		if ( $condition_number ) {
			...
			if ( ! empty( $single_row[ $this->get_label_condition_id() ] ) ) {
				...
				if ( ! empty( $this->available_conditions[ $condition_id ] ) ) {
					...
					$conditions[ $condition_number ] = $this->process_fields( [ Rule::CONDITION_ID => $condition_id ], $condition_fields, [ $this, 'get_label_condition_field' ], $single_row );
