...
class ShippingMethodExport extends CSVAbstract {
	...
	private function get_method_data( $row_number ) {
		...
		if ( 1 === $row_number ) {
			foreach ( $this->get_shipping_method_settings() as $key => $details ) {
				$data[ $this->get_label_shipping_method_field( $key ) ] = $this->json_encode_if_array( $this->shipping_method->get_instance_option( $key ) );
			}
		}
	 ...
	 * @return string
	 */
	private function json_encode_if_array( $value ) {
		if ( is_array( $value ) ) {
	...
	private function get_shipping_method_settings() {
		return array_filter(
			...
			function ( $value ) {
				return ! in_array( $value['type'], [ 'title', 'shipping_rules' ] );
			}
		);
	}
}
